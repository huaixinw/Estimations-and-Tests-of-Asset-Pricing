# -*- coding: utf-8 -*-
"""
Fama-Macbeth regression and GMM estimation
replication of:
...
#------------------------------------------------------------------------------   
    ...
    gamma = pd.DataFrame()
    gamma_intercept = pd.DataFrame()
    error_cs = pd.DataFrame()
    R_square = pd.DataFrame()
    
    
    # calculate period mean of gamma   
    gamma_hat = gamma.mean().reset_index().rename(columns={'index':'factor'})
    gamma_intercept_hat = gamma_intercept.mean()
    gamma_var = (gamma.var()/len(assets)).reset_index().rename(columns={'index':'factor'})
    gamma_tstat = gamma_hat[0]/np.sqrt(gamma_var[0])
    # the s-statistics of the intercept?
    # calculate period mean of pricing error(alpha)
    error_cs.columns = ['alpha','period','asset']
    error_hat = error_cs.groupby(['asset'])['alpha'].mean().reset_index().\
        rename(columns={'alpha':'alpha_hat'})
    # calculate mean absolute pricing error(MAPE)
    MAPE = error_cs['alpha'].abs().mean()
    # calculate chi2 statistics (Cochrane p247)
    error_cs1 = pd.merge(error_cs,error_hat,how='left',on=['asset'])
    error_cs1['alpha_dev'] = error_cs1['alpha']-error_cs1['alpha_hat']
    alpha_dev = error_cs1.pivot(index='period',columns='asset',values='alpha_dev')
    alpha_hat = error_hat['alpha_hat'].to_numpy()
    cov_alpha = alpha_dev.cov()/len(assets)
    chi2_alpha = np.dot(np.dot(alpha_hat.T,np.linalg.inv(cov_alpha.to_numpy())),alpha_hat)
    # R2
    '''
    R2 = 1-(1-R_square.mean())*(N-1)/(N-k-1)
    TSS = np.square(exp_ret.sub(exp_ret.mean(axis=0)).to_numpy()).sum()
    fitted = np.dot(gamma.to_numpy(),beta.to_numpy().T)+gamma_intercept.to_numpy()
    ESS = np.square(fitted-fitted.mean(axis=0)).sum()
    ESS/TSS
    '''
